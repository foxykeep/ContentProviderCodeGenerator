...
public class CodeBuilder {
    ...
    private static final String[] GENERATED_CODE_TEXT = {
            "/**", " * This class was generated by DatabaseCodeGenerator", " */"
    };
    ...
    public void startClass(final String className, final String superClassName, final String interfaceName, final boolean isPublic,
            final boolean isStatic, final boolean isFinal, final boolean isAbstract) {
        addLine("%1$s%2$s%3$s%4$s class %5$s%6$s%7$s {\n", isPublic ? "public " : "private ", isStatic ? "static " : "", isFinal ? "final " : "",
                isAbstract ? "abstract" : "", className, superClassName != null ? " extends " + superClassName : "",
                interfaceName != null ? " implements " + interfaceName : "");
        indent();
    }
    ...
    public void addFieldCustom(final boolean isPublic, final boolean isStatic, final boolean isFinal, String type, final String name,
            final String value) {
        final int bracketIndex = type.lastIndexOf('[');
        String importType = bracketIndex != -1 ? type.substring(0, bracketIndex) : type;
        if (!importType.equals("int") && !importType.equals("long") && !importType.equals("float") && !importType.equals("boolean")
                && !importType.equals("string") && !importType.equals("String")) {
            addImport(importType);
            type = type.substring(type.lastIndexOf('.') + 1);
        } else if (type.equals("string")) {
            type = "String";
        }
        addLine("%1$s %2$s%3$s%4$s %5$s%6$s;", isPublic ? "public" : "private", isStatic ? "static " : "", isFinal ? "final " : "", type, name,
                value != null ? " = " + value : "");
    }
